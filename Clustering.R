...
install.packages(c("dplyr", "reshape2", "ggthemes", "gganimate", "cluster", "Rtsne", "knitr", "dbscan"))
...
population_data %>% 
  ...
  theme_void() +
  labs(title = "World Population Percentage by Continent") +  # Add heading
  geom_text(data = population_data %>% slice_head(n = 5), aes(label = Country), hjust = -0.5, size = 3)  # Add labels for the first 5 countries
...
ggplot(dfs, aes(x = Cluster_num, y = Silhouette_width)) + 
  ...
  geom_text(data = dfs %>% slice_head(n = 5), aes(label = Cluster_num), hjust = -0.5, size = 3) 
...
ggplot(aes(x = X, y = Y), data = tsne_data) +
  ...
  theme_bw() +
  labs(title = "t-SNE Visualization with Cluster Labels")  
...
ggplot(aes(x = X, y = Y), data = tsne_data) +
  ...
  scale_color_brewer(palette = "Dark2") +
  theme_bw() +
  labs(title = "t-SNE Visualization with Hierarchical Cluster Labels")  
...
tsne_data %>% 
  ...
  scale_fill_brewer(palette = "Dark2") +
  theme_bw() +
  labs(title = "Distribution of DBSCAN Clusters")  
...
ggplot(aes(x = X, y = Y), data = tsne_data) +
  ...
  scale_color_brewer(palette = "Dark2") +
  theme_bw() +
  labs(title = "t-SNE Visualization with DBSCAN Cluster Labels")  
...
tsne_data %>% 
  ...
  scale_fill_brewer(palette = "Dark2") +
  theme_bw() +
  labs(title = "Distribution of K-means Clusters")  
...
ggplot(aes(x = X, y = Y), data = tsne_data) +
  ...
  scale_color_brewer(palette = "Dark2") +
  theme_bw() +
  labs(title = "t-SNE Visualization with K-means Cluster Labels")  
